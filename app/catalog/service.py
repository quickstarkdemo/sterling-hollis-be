...
def _conditions(filters: ProductFilters, *, include_search: bool = True):
    ...
    if filters.category:
        conditions.append(func.lower(CatalogProductModel.category) == filters.category.lower())
    if filters.brand:
        conditions.append(func.lower(CatalogProductModel.brand) == filters.brand.lower())
    if filters.size:
    ...
    if filters.availability:
        conditions.append(func.lower(StoreInventory.availability) == filters.availability.lower())
    ...
    if filters.in_stock_only:
        conditions.append(and_(func.lower(StoreInventory.availability) == "in stock", StoreInventory.inventory_qty > 0))
...
def _filtered_product_id_query(filters: ProductFilters):
    query = (
        select(
            ...
            func.coalesce(func.sum(StoreInventory.inventory_qty), 0).label("inventory_units"),
            func.coalesce(func.max(StoreInventory.objective_weight), 0).label("objective_weight"),
        )
        .join(ProductVariant, ProductVariant.catalog_product_id == CatalogProductModel.id)
    ...
    if filters.sort == ProductSort.price_asc:
        query = query.order_by(func.min(ProductVariant.price_min).asc(), CatalogProductModel.id.asc())
    elif filters.sort == ProductSort.price_desc:
        query = query.order_by(func.max(ProductVariant.price_max).desc(), CatalogProductModel.id.asc())
    elif filters.sort == ProductSort.inventory_desc:
        query = query.order_by(func.coalesce(func.sum(StoreInventory.inventory_qty), 0).desc(), CatalogProductModel.id.asc())
    elif filters.sort == ProductSort.newest:
        query = query.order_by(CatalogProductModel.id.desc())
    else:
        query = query.order_by(func.coalesce(func.max(StoreInventory.objective_weight), 0).desc(), CatalogProductModel.id.asc())
    return query


def _inventory_rows(db: Session, variant_ids: list[str], *, store_id: str | None = None) -> list[StoreInventory]:
    if not variant_ids:
        return []
    ...
    return db.scalars(query.order_by(StoreInventory.store_id.asc(), StoreInventory.size.asc())).all()


def _variant_images(product: CatalogProductModel, variant: ProductVariant) -> ProductImages:
...
def _summary(rows: list[StoreInventory]) -> ProductInventorySummary:
    ...
    in_stock_rows = [row for row in rows if is_in_stock(row.availability, row.inventory_qty)]
...
def product_to_catalog(
    ...
    *,
    ...
    inventory_by_variant: dict[str, list[StoreInventory]] = {variant_id: [] for variant_id in variant_ids}
    ...
    default_variant = max(
        variants,
        key=lambda variant: (
            sum(int(row.inventory_qty or 0) for row in inventory_by_variant.get(variant.id, [])),
            -float(variant.price_min),
            variant.id,
        ),
        default=None,
    )
    images = _variant_images(product, default_variant) if default_variant else ProductImages()
...
def list_categories(db: Session, *, store_id: str | None = None) -> CategoryListResponse:
    conditions = []
    if store_id:
...
def _facet_group(db: Session, filters: ProductFilters, name: str, column) -> ProductFacetGroup:
    rows = db.execute(
        ...
        .group_by(column)
        .order_by(func.count(func.distinct(CatalogProductModel.id)).desc(), column.asc())
        .limit(20)
    ).all()
    return ProductFacetGroup(
        name=name,
        values=[ProductFacetValue(value=str(row[0]), count=int(row[1] or 0)) for row in rows if row[0]],
    )


def list_products(db: Session, filters: ProductFilters, *, include_facets: bool = True) -> ProductListResponse:
    product_id_query = _filtered_product_id_query(filters)
    total = db.scalar(select(func.count()).select_from(product_id_query.subquery())) or 0
    product_ids = [
        row.product_id
        for row in db.execute(product_id_query.limit(filters.limit).offset(filters.offset)).all()
    ]
    product_map = {
        product.id: product
    ...
    return ProductListResponse(
        items=[product_to_catalog(db, product, store_id=filters.store_id) for product in products],
        total=int(total),
        limit=filters.limit,
...
def get_product_detail(db: Session, product_id: str, *, store_id: str | None = None) -> ProductDetailResponse | None:
...
def related_products(db: Session, product_id: str, *, limit: int = 12) -> ProductListResponse | None:
    normalized_id = _product_id_for_legacy_id(db, product_id)
    product = db.get(CatalogProductModel, normalized_id)
    if not product:
        return None
    ...
    return ProductListResponse(items=items, total=len(items), limit=limit, offset=0, facets=[])


def recommend_products(db: Session, req: ProductRecommendationRequest) -> ProductRecommendationResponse:
    if req.store_id and req.customer_id:
        ...
        for row in rows:
            ...
            if len(recommendations) >= req.top_k:
                break
        return ProductRecommendationResponse(recommendations=recommendations, strategy=strategy)
